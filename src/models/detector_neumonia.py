...
Implementada con PySide6 (Qt) para una apariencia moderna y profesional.
Sigue convenciones PEP 8 (nombres, indentación, líneas ≤79 caracteres,
docstrings y agrupación de importaciones).
"""
...
# Tamaño por defecto para escalar imágenes en pantalla (máx. por lado).
IMAGE_DISPLAY_SIZE = 480
# Tamaño mínimo del recuadro cuando no hay imagen (placeholder).
IMAGE_PLACEHOLDER_MIN = 200
...
def numpy_rgb_to_qpixmap(
    arr,
    size: int = IMAGE_DISPLAY_SIZE,
) -> QPixmap:
    """
    Convierte un array numpy RGB (H, W, 3) uint8 a QPixmap escalado.

    Si el array es 2D (escala de grises), se convierte a RGB antes.

    Args:
        arr: Array numpy (H, W) o (H, W, 3), dtype uint8.
        size: Longitud máxima del lado para el escalado.

    Returns:
        QPixmap listo para mostrar en la interfaz.
    """
    h, w = arr.shape[:2]
    if arr.ndim == 2:
...
def _clear_image_label(
    ...
    El tamaño mínimo del recuadro se reduce para el estado vacío.

    Args:
        label_widget: QLabel a limpiar.
        placeholder_text: Texto a mostrar cuando no hay imagen.
    """
    label_widget.setPixmap(QPixmap())
    label_widget.setText(placeholder_text)
    label_widget.setMinimumSize(IMAGE_PLACEHOLDER_MIN, IMAGE_PLACEHOLDER_MIN)


# Estilo moderno y limpio (tema claro profesional). Líneas largas en strings
# de estilos son aceptables para legibilidad del CSS.
STYLESHEET = """
QMainWindow { background-color: #f5f6f8; }
QWidget#central { background-color: #f5f6f8; }
QLabel#titleLabel {
    color: #1a237e;
    font-size: 22px;
    font-weight: bold;
...
QPushButton#primary { background-color: #3949ab; color: white; }
QPushButton#primary:hover { background-color: #303f9f; }
...
QPushButton#secondary:hover { background-color: #cfd8dc; }
QPushButton#success { background-color: #43a047; color: white; }
QPushButton#success:hover { background-color: #388e3c; }
QPushButton#danger { background-color: #e53935; color: white; }
...
class DetectorWindow(QMainWindow):
    ...
    def __init__(self):
        """Inicializa la ventana, widgets y conexiones."""
        super().__init__()
        self.array = None
        self.label = ""
        self.proba = 0.0
        ...
        lbl_orig = QLabel("Radiografía (entrada)")
        lbl_orig.setObjectName("sectionLabel")
        lbl_heat = QLabel("Explicación (heatmap Grad-CAM)")
        ...
        self.img_original.setMinimumSize(
            IMAGE_PLACEHOLDER_MIN,
            IMAGE_PLACEHOLDER_MIN,
        )
        self.img_original.setText(self.PLACEHOLDER_ORIGINAL)
        self.img_original.setSizePolicy(
        ...
        self.img_heatmap.setSizePolicy(
            QSizePolicy.Expanding,
            QSizePolicy.Expanding,
        )
        ...
        # Solo dígitos (hasta 15). Regex evita overflow de QIntValidator.
        ...
        self.btn_predict = QPushButton("Predecir (IA)")
    ...
    def load_image(self):
        """
        Abre el diálogo para elegir una imagen y la muestra.

        Habilita el botón Predecir y limpia el heatmap anterior.
        """
        path, _ = QFileDialog.getOpenFileName(
            ...
            "DICOM (*.dcm);;JPEG (*.jpg *.jpeg);;PNG (*.png)",
        )
        if not path:
            return
        try:
    ...
    def run_prediction(self):
        """Lanza la predicción en segundo plano (worker)."""
    ...
    def _on_prediction_finished(
        ...
        """
        Slot llamado cuando el worker termina con éxito.

        Actualiza etiquetas, probabilidad y muestra el heatmap.
        """
        self.progress.setVisible(False)
        self.btn_predict.setEnabled(True)
        self.label = label
    ...
    def _on_prediction_error(self, msg: str):
        """Slot llamado cuando el worker falla; muestra el error."""
        self.progress.setVisible(False)
        self.btn_predict.setEnabled(True)
    ...
    def save_results_csv(self):
        ...
        Muestra advertencia si no hay cédula; pide cédula antes de guardar.
        """
    ...
    def export_pdf(self):
        """
        Genera un PDF con imagen original, heatmap y datos del paciente.

        Requiere cédula y que se haya ejecutado al menos una predicción.
        """
        cedula = self.cedula_edit.text().strip()
        if not cedula:
            QMessageBox.warning(
    ...
    def _build_pdf(self, cedula: str) -> str:
        """
        Genera el archivo PDF con imagen original, heatmap y texto.

        No usa captura de pantalla; compone la imagen con PIL e img2pdf.

        Args:
            cedula: Cédula del paciente para el nombre del archivo.
        ...
        draw.text(
            (gap, 50),
            f"Resultado: {self.label}  |  "
            f"Probabilidad: {self.proba * 100:.2f}%",
            fill=(60, 60, 60),
            font=font_small,
        )
    ...
    def clear_all(self):
        ...
        Restaura los placeholders y deshabilita Predecir.
        """
        ...
        QMessageBox.information(
            self,
            "Listo",
            "Se limpió la pantalla correctamente.",
        )


def main():
    """
    Punto de entrada: crea la aplicación Qt y la ventana principal.

    Returns:
        Código de salida de QApplication.exec().
    """
    app = QApplication([])
    app.setApplicationName("Detección de neumonía")
    win = DetectorWindow()
    win.show()
    return app.exec()
